...
/** チャンク間での話者統合が有効か取得 (config から読み込み、デフォルト true) */
function isMergeAcrossChunksEnabled(): boolean {
  try {
    const config = loadConfig();
    return config.speaker?.mergeAcrossChunks ?? true;
  } catch {
    return true;
  }
}
...
/**
 * 2つの embedding の加重平均を計算する。
 * 既存の embedding に新しい embedding を count 回分の重みで累積平均する。
 */
function averageEmbeddings(existing: number[], newEmb: number[], existingCount: number): number[] {
  if (existing.length !== newEmb.length) return newEmb;
  const totalCount = existingCount + 1;
  return existing.map((val, i) => {
    const newVal = newEmb[i] ?? 0;
    return (val * existingCount + newVal) / totalCount;
  });
}
  ...
  for (const [label, emb] of Object.entries(embeddings)) {
    ...
    if (matchedName) {
      ...
      if (meta) {
        ...
        // embedding を累積平均で更新 (チャンク間統合が有効な場合)
        if (isMergeAcrossChunksEnabled()) {
          const existingEmb = registeredEmbeddings[matchedName];
          if (existingEmb) {
            registeredEmbeddings[matchedName] = averageEmbeddings(
              existingEmb,
              emb,
              meta.occurrenceCount - 1,
            );
            consola.debug(
              `Updated ${matchedName} embedding (averaged with count ${meta.occurrenceCount})`,
            );
          } else {

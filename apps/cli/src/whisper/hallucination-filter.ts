...
export const HALLUCINATION_PATTERNS: RegExp[] = [
  ...
  // 繰り返しノイズ: 「はっ」の5回以上の繰り返し + 「あ」の5回以上の繰り返し
  // 「でローカルで検証...」のような正当なコンテンツが「はっはっはっはっはっ...」(50+個)で圧倒される混合ハルシネーション
  // 「はっ」は笑い/フィラー音として認識されるが、5回以上の連続繰り返しはノイズの強い指標
  // 例: 「はっはっはっはっはっはっはっはっはっはっ」など
  /^(はっ{5,}|あ{5,}[\s.]*)+$/,

  // 繰り返しノイズ: 「ジョブアンテナー」の極端な繰り返し (5回以上)
  // 無音区間での極度なハルシネーション。「ジョブアンテナー」が24回以上連続繰り返される
  // 通常会話では起こりえない極端なフレーズ繰り返し。音声ノイズが単一フレーズ単位で繰り返される典型的なパターン
  // 例: 「ジョブアンテナージョブアンテナー...」(24+回の連続繰り返し)
  /^([ジョブアンテナー]+[\s.]*){5,}$/,

  // 不完全な発話: 小さなひらがな + 「がいなくて」+ 「あれ?」の組み合わせ
  // 無音区間での断片的で不連貫なハルシネーション。「あ なんか今日がいなくて あれ?」のような
  // 主語の喪失と不完全な疑問文で意味を持たない発話パターン
  // 例: 「あ なんか今日がいなくて あれ?」, 「い 何かがいなくて あれ?」など
  /^(^[ぁ-ん]\s+[ぁ-ん]+がいなくて\s+あれ\?$[\s.]*)+$/,

  // スペース区切りの「ジョブアンテナー」の繰り返し (5回以上)
  // 「ジョブアンテナー」が半角スペースで区切られて5回以上連続繰り返される
  // 例: 「ジョブアンテナー ジョブアンテナー ジョブアンテナー ジョブアンテナー ジョブアンテナー」
  // 無音区間での特性的なハルシネーション。セマンティック内容なし
  /^(ジョブアンテナー\s+){5,}$/,
];
